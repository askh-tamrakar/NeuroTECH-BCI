...
try:
    ...
    pylsl = None
    LSL_AVAILABLE = False
    print("pylsl not available - LSL streams will be disabled. Install with `pip install pylsl`.")
...
class LSLStreamer:
    def __init__(self, name: str, channel_types: List[str], channel_labels: Optional[List[str]] = None,
        ...
        if LSL_AVAILABLE:
            ...
            print(f"LSL not available - '{self.name}' stubbed")
    ...
    def push_sample(self, sample: List[float], ts: Optional[float] = None):
        ...
        try:
            ...
            print(f"push_sample error for '{self.name}': {e}")
    ...
    def push_chunk(self, chunk: List[List[float]], ts: Optional[float] = None):
        ...
        try:
            ...
            print(f"push_chunk error for '{self.name}': {e}")
