...
class EMGFilterProcessor:
    ...
    def update_config(self, config: dict, sr: int):
        ...
        if old_state != new_state:
            print(f"Config changed ({self.channel_key}) -> HP:{self.hp_cutoff} Notch:{self.notch_enabled}({self.notch_freq}Hz) Env:{self.envelope_enabled} ({self.envelope_cutoff}Hz)")
            self._design_filters()
