        ...
        >
            ...
            {!showSidebar && (
                <div className="flex flex-col items-center justify-around py-3 w-full animate-fade-in shrink-0 h-full overflow-visible">
                    ...
                    <button onClick={() => setShowSidebar(true)} className="hover:bg-white/10 rounded-full transition-colors" title="Expand Sidebar">
                    ...
                    <button
                        onClick={() => { const v = !useML; setUseML(v); setShowSidebar(true); }}
                    ...
                    >
                        <Brain size={30} className={useML ? 'animate-pulse' : ''} />
                        <div className="absolute left-14 top-1/2 -translate-y-1/2 bg-[var(--surface)] border border-[var(--primary)]/40 px-3 py-1.5 rounded-lg text-[10px] font-black text-[var(--primary)] whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none z-50">
                    ...
                    <div className="flex flex-col items-center cursor-default group relative w-full" title="Signal Frequency">
                        ...
                        <div className="absolute left-14 top-1/2 -translate-y-1/2 bg-[var(--surface)] border border-[var(--border)] px-1.5 py-1.5 rounded-lg text-xs font-bold text-[var(--text)] whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none z-50">Live Signal (Hz)</div>
                    </div>
                    ...
                    <button onClick={() => setShowSidebar(true)} title="System Activity" className="hover:text-[var(--primary)] transition-colors group relative">
                        ...
                        {logs.length > 0 && <span className="absolute -top-1 -right-1 w-2 h-2 bg-[var(--primary)] rounded-full animate-pulse blur-[1px]" />}
                        <div className="absolute left-14 top-1/2 -translate-y-1/2 bg-[var(--surface)] border border-[var(--border)] px-3 py-1.5 rounded-lg text-xs font-bold text-[var(--text)] whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none z-50">System Activity Logs</div>
                    </button>

                    <div className="w-full h-px bg-[var(--border)]/80 shrink-0" />

                    <button onClick={globalRunning ? stopFlicker : startFlicker} title="Start/Stop" className={`transition-colors group relative p-2 rounded-full ${globalRunning ? 'text-red-500 hover:bg-red-500/20' : 'text-green-500 hover:bg-green-500/20'}`}>
                        {globalRunning ? <Square size={28} /> : <Play size={28} />}
                        <div className="absolute left-14 top-1/2 -translate-y-1/2 bg-[var(--surface)] border border-[var(--border)] px-1.5 py-1.5 rounded-lg text-xs font-bold text-[var(--text)] whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none z-50">{globalRunning ? 'Stop Simulation' : 'Start Simulation'}</div>
                    </button>
                    ...
                    {!globalRunning && (
                        <button onClick={runProtocol} title="Run Protocol" className="transition-colors group relative p-2 rounded-full text-[var(--primary)] hover:bg-[var(--primary)]/20">
                            <Zap size={28} />
                            <div className="absolute left-14 top-1/2 -translate-y-1/2 bg-[var(--surface)] px-3 py-1.5 rounded-lg text-xs font-bold text-[var(--text)] whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none z-50">Run Protocol</div>
                        </button>
                    )}
                    ...
                    <button onClick={() => setShowSidebar(true)} title="Targets Settings" className="hover:text-[var(--primary)] transition-colors group relative">
                        ...
                        <div className="absolute left-14 top-1/2 -translate-y-1/2 bg-[var(--surface)] border border-[var(--border)] px-3 py-1.5 rounded-lg text-xs font-bold text-[var(--text)] whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none z-50">Targets Settings</div>
                    </button>
                    ...
                    {configs.filter(c => c.enabled).map(cfg => (
                        <div key={cfg.id} className="flex flex-col items-center group relative cursor-help" title={cfg.label}>
                            ...
                            <div className="absolute left-14 top-1/2 -translate-y-1/2 bg-[var(--surface)] border border-[var(--border)] px-3 py-1.5 rounded-lg text-xs font-bold text-[var(--text)] whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none z-50">{cfg.label} ({cfg.freq}Hz)</div>
                        </div>
                    ))}
                    ...
                    <div className="flex flex-col w-full items-center shrink-0">
                        <button className={`w-[42px] h-[42px] flex items-center justify-center rounded-full border transition-all cursor-default shadow-sm group relative ${isConnected ? 'bg-green-500/10 border-green-500/30 text-green-500' : 'bg-red-500/10 border-red-500/30 text-red-500'}`} title={isConnected ? 'Sensor Connected' : 'Sensor Disconnected'}>
                            ...
                            <div className="absolute left-14 top-1/2 -translate-y-1/2 bg-[var(--surface)] border border-[var(--border)] px-3 py-1.5 rounded-lg text-xs font-bold text-[var(--text)] whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none z-50">Sensor Status</div>
                        </button>
                    </div>
                </div>
            )}
            ...
            <div className={`flex-grow flex flex-col overflow-hidden p-4 gap-3 font-mono transition-opacity duration-300 w-80 shrink-0 ${!showSidebar ? 'opacity-0 h-0 hidden' : 'opacity-100'}`}>
                ...
                <div className="flex items-center justify-between shrink-0 mb-2">
                    ...
                    <div className="flex gap-2">
                        <button onClick={onBackToMenu} className="p-2 hover:bg-white/10 rounded-full transition-colors" title="Back to Dashboard Menu">
                            ...
                            <div className="space-y-1">
                                {configs.map(cfg => (
                                    <div key={cfg.id} className={`flex items-center justify-between px-3 py-3 relative hover:bg-white/5 rounded-lg transition-colors group ${!cfg.enabled && 'grayscale opacity-50'}`}>
                                        ...
                                        {isSyncing && lastModifiedTargetId === cfg.id ? <span className="w-1.5 h-1.5 rounded-full bg-[var(--primary)] animate-pulse shadow-[0_0_5px_currentColor] shrink-0" /> : <span className="w-1.5 h-1.5 shrink-0" />}
                                ...
                                return (
                                    <div key={cfg.id} className={`p-3 rounded-xl border transition-all space-y-2 shrink-0 relative transform-gpu ${cfg.enabled ? 'bg-[var(--bg)]/50 border-[var(--border)]' : 'bg-[var(--bg)]/20 border-[var(--border)]/30 grayscale opacity-60'}`} style={{ zIndex: openDropdownId === cfg.id ? 100 : 50 - index }}>
                                        <div className="flex items-center justify-between gap-1">
                                            ...
                                            {isSyncing && lastModifiedTargetId === cfg.id ? <span className="w-1.5 h-1.5 rounded-full bg-[var(--primary)] animate-pulse shadow-[0_0_5px_currentColor] shrink-0 mx-1" /> : <span className="w-1.5 h-1.5 shrink-0 mx-1" />}
