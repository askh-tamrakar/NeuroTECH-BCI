            ...
            {Object.entries(importances).sort(([, a], [, b]) => b - a).map(([name, imp]) => (
                <li key={name} className="flex items-center text-[var(--text)] group hover:bg-[var(--bg)]/30 rounded px-1 py-0.5">
                    ...
                    <div className="flex-1 h-2 bg-[var(--bg)] rounded-full ml-2 overflow-hidden border border-[var(--border)]">
                        <div className="h-full bg-[var(--primary)] group-hover:bg-[var(--accent)] transition-colors" style={{ width: `${Math.min(100, Math.max(0, imp * 100))}%` }}></div>
                    </div>
                    <span className="ml-2 text-[13px] w-10 text-right font-mono">{(imp * 100).toFixed(1)}%</span>
                </li>
            ))}
        </ul>
    </div>
...
const EEGWorkspaceCard = ({ result }) => (
    <div className="card h-full flex flex-col p-5 bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-sm">
        ...
        <div className="grid gap-4 py-4 md:grid-cols-2 h-full">
            <div className="rounded-xl border border-[var(--border)] bg-[var(--bg)]/30 p-4">
                ...
                <div className="mt-2 text-3xl font-black text-[var(--primary)]">{result?.model_name || 'eeg_lda'}</div>
                <div className="mt-3 flex flex-col gap-2 text-sm text-[var(--text)]">
                    ...
                    <div className="flex justify-between items-center">
                        <span className="text-[var(--muted)]">Test Samples</span>
                        <span className="font-mono text-[var(--primary)]">{result?.n_samples ?? '--'}</span>
                    </div>
                    <div className="flex justify-between items-center">
                        <span className="text-[var(--muted)]">Total Rows</span>
                        <span className="font-mono text-[var(--primary)]">{result?.total_samples ?? result?.n_samples ?? '--'}</span>
                    </div>
                </div>
            </div>
            <div className="rounded-xl border border-[var(--border)] bg-[var(--bg)]/30 p-4">
                <div className="text-xs uppercase tracking-widest text-[var(--muted)]">LDA Snapshot</div>
                <div className="mt-3 grid grid-cols-2 gap-3">
                    <div className="rounded-lg border border-[var(--border)] bg-[var(--bg)]/35 p-3">
                    ...
                    <div className="rounded-lg border border-[var(--border)] bg-[var(--bg)]/35 p-3">
                        <div className="text-[10px] uppercase tracking-[0.24em] text-[var(--muted)]">Components</div>
                        <div className="mt-2 text-2xl font-black text-[var(--text)]">{result?.visualization?.component_count ?? '--'}</div>
                    </div>
                </div>
                <div className="mt-3 rounded-lg border border-[var(--border)] bg-[var(--bg)]/35 p-3 text-xs leading-5 text-[var(--muted)]">
                    EEG uses the score-space SSVEP feature vector. This workspace is separate from the EMG/EOG RF workflow and keeps LDA-specific diagnostics only.
                </div>
            </div>
        </div>
    </div>
...
const EEGFeatureSetCard = ({ result }) => {
    const features = result?.feature_order || [
        ...
        <div className="card h-full flex flex-col p-4 bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-sm">
            ...
            <div className="py-4 flex-1 flex flex-col gap-3">
                ...
                <div className="rounded-lg border border-[var(--border)] bg-[var(--bg)]/35 p-3 text-xs leading-5 text-[var(--muted)]">
                    Session-based EEG LDA does not expose tree inspection. Instead, the cards below show class centroids and class signatures derived from the trained discriminant space.
                </div>
            </div>
        </div>
    );
    ...
    <div className="card h-full flex flex-col p-4 bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-sm">
        ...
        <div className="py-4 space-y-4 flex-1">
            <div>
                ...
                <input type="range" min="0.1" max="0.5" step="0.05" name="test_size" value={params.test_size} onChange={onChange} className="w-full accent-[var(--primary)]" />
            </div>
            <div>
                <div className="text-xs text-[var(--text)] mb-1">Solver</div>
                <CustomSelect
            ...
            <div>
                <div className="text-xs text-[var(--text)] mb-1">Shrinkage</div>
                <CustomSelect
            ...
            <div className="rounded-lg border border-[var(--border)] bg-[var(--bg)]/40 p-3 text-xs leading-5 text-[var(--muted)]">
                EIGEN + AUTO keeps shrinkage enabled and still exposes a projection space for visualization. Switch to SVD if you want the classic unregularized solution.
            </div>
        </div>
    </div>
);

const EEGTrainingDataCard = ({ result, selectedSessionName }) => (
    <div className="card h-full flex flex-col p-4 bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-sm">
        <h3 className="text-base flex items-center font-bold text-[var(--muted)] uppercase tracking-widest border-b border-[var(--border)] pb-2">
        ...
        <div className="py-4 space-y-3 text-sm text-[var(--text)]">
            <div className="flex justify-between items-center">
                ...
                <span className="font-mono text-[var(--primary)] truncate max-w-[11rem]" title={selectedSessionName}>{selectedSessionName}</span>
            </div>
            <div className="flex justify-between items-center">
                <span className="text-[var(--muted)]">Model File</span>
                <span className="font-mono text-[var(--primary)] truncate max-w-[11rem]" title={result?.model_path}>{result?.model_path ? result.model_path.split(/[\\/]/).pop() : '--'}</span>
            </div>
            <div className="flex justify-between items-center">
                <span className="text-[var(--muted)]">Source</span>
                <span className="font-mono text-[var(--primary)] truncate max-w-[11rem]" title={result?.source}>{result?.source || 'N/A'}</span>
            </div>
            <div className="rounded-lg border border-[var(--border)] bg-[var(--bg)]/40 p-3 text-xs leading-5 text-[var(--muted)]">
                Train on one session for subject-specific tuning. Use the full EEG table for a broader but less controlled model.
            </div>
        </div>
    </div>
);
...
const EEGLDAVisualizationCard = ({ result }) => {
    ...
    return (
        <div className="card h-full flex flex-col p-4 bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-sm">
            <h3 className="text-base flex items-center font-bold text-[var(--muted)] uppercase tracking-widest border-b border-[var(--border)] pb-2">
                <PieChart className="mr-2 w-4 h-4" /> LDA Signature View
            </h3>
            <div className="py-4 flex-1 grid gap-4 lg:grid-cols-2">
                <div className="space-y-2">
                    <div className="text-xs uppercase tracking-widest text-[var(--muted)]">Class Centroids</div>
                    {centroids.length ? centroids.map((centroid) => (
                        <div key={centroid.label} className="rounded-lg border border-[var(--border)] bg-[var(--bg)]/30 px-3 py-2">
                            <div className="flex items-center justify-between text-sm font-bold">
                                <span>{centroid.name}</span>
                                <span className="text-[var(--primary)]">{centroid.count} rows</span>
                            </div>
                            <div className="mt-2 flex gap-2 text-xs font-mono text-[var(--muted)]">
                                <span>LD1 {centroid.ld1?.toFixed?.(2) ?? '0.00'}</span>
                                <span>LD2 {centroid.ld2?.toFixed?.(2) ?? '0.00'}</span>
                            </div>
                        </div>
                    )) : (
                ...
                <div className="space-y-2">
                    <div className="text-xs uppercase tracking-widest text-[var(--muted)]">Class Signatures</div>
                    {signatures.length ? signatures.map((signature) => (
                        <div key={signature.label} className="rounded-lg border border-[var(--border)] bg-[var(--bg)]/30 px-3 py-3">
                            <div className="text-sm font-bold text-[var(--text)] mb-2">{signature.name}</div>
                            <div className="space-y-2">
                                {signature.signature.map((feature) => (
                                    <div key={`${signature.label}-${feature.feature}`} className="flex items-center gap-2">
                                        <span className="w-24 text-[11px] font-mono text-[var(--muted)] truncate">{feature.feature}</span>
...
const ConfusionMatrixCard = ({ matrix, labels, n_samples, sensor }) => (
    <div className="card h-full flex flex-col p-4 bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-sm">
        ...
        <div className="flex-grow overflow-auto flex flex-col h-full py-4 relative [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {matrix ? (
                <table className="min-w-[42rem] w-full h-full text-[16px] text-center text-[var(--text)] border-collapse table-fixed">
                    <thead>
                        <tr>
                                    ...
                                    <div className="col-span-12 md:col-span-9 row-span-6 min-h-0 flex flex-col overflow-hidden">
                                        <div className="grid grid-cols-12 grid-rows-6 gap-4 h-full">
                                            <div className="col-span-12 lg:col-span-8 row-span-2 min-h-0 flex flex-col overflow-hidden">
                                                <EEGWorkspaceCard result={activeResult || activeEvalResult} />
                                            </div>
                                            ...
                                            <div className="col-span-12 lg:col-span-4 row-span-4 min-h-0 flex flex-col overflow-hidden">
                                                <ConfusionMatrixCard
                                                    matrix={(activeResult || activeEvalResult).confusion_matrix}
                                                    labels={(activeResult || activeEvalResult).labels || []}
                                                    n_samples={(activeResult || activeEvalResult).n_samples}
